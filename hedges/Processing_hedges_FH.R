...
for (row_start in seq(1, nrow_chm, by = section_size - overlap)) {
  for (col_start in seq(1, ncol_chm, by = section_size - overlap)) {
    ...
    canopy_area <- stars:::st_as_sf.stars(stars::st_as_stars(filt_max_class), point = FALSE, merge = TRUE, connect8 = TRUE) %>% st_buffer(0.01) %>% st_union() %>% st_cast('POLYGON') %>% st_as_sf() 
    ...
    # Find hedges
    hedges <- NULL
    for (pol in 1:length(canopy_area$x)) {
      ...
      narrow_parts1 <- st_difference(poly, regrown) %>% st_union() %>% st_cast('POLYGON') %>% st_as_sf()
      ...
      narrow_parts1$area_m <- narrow_parts1 %>% st_area() %>% units::drop_units()
      ...
      # remove pixels over 6m
      poly_cleaned <- st_difference(narrow_parts1, mask_poly) %>% st_union() %>% st_cast('POLYGON')
      ...
      # Subtract to get narrow border regions
      narrow_parts2 <- st_difference(poly_cleaned, regrown2) %>% st_union() %>% st_cast('POLYGON') %>% st_as_sf()
      ...
      narrow_parts2$area_m <- narrow_parts2 %>% st_area() %>% units::drop_units()
      ...
      narrow_parts <- st_simplify(narrow_parts, dTolerance = 0.05, preserveTopology = TRUE)
